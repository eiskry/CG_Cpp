	...
	for (unsigned int i = 0; i < g_ControlPoints.size(); i++) {
		if (i % 3 == 0 && i != 0) {
			...
			for (double t = 0; t <= 1.01; t += 0.01) {
				glVertex2d(
					(1 - t) * (1 - t) * (1 - t) * g_ControlPoints[i - 3].x + 3 * t * (1 - t) * (1 - t) * g_ControlPoints[i - 2].x + 3 * t * t * (1 - t) * g_ControlPoints[i - 1].x + t * t * t * g_ControlPoints[i].x,
					(1 - t) * (1 - t) * (1 - t) * g_ControlPoints[i - 3].y + 3 * t * (1 - t) * (1 - t) * g_ControlPoints[i - 2].y + 3 * t * t * (1 - t) * g_ControlPoints[i - 1].y + t * t * t * g_ControlPoints[i].y
				);
			}
		}
	}
	...
	for (unsigned int i = 0; i < g_ControlPoints.size(); i++) {
		if (i % 3 == 0 && i != 0) {
			for (double t = 0; t <= 1.01; t += 0.01) {
				...
				Vector2d g_diff(
					-3 * (1 - t) * (1 - t) * g_ControlPoints[i - 3].x + (9 * t * t - 12 * t + 3) * g_ControlPoints[i - 2].x + (-9 * t * t + 6 * t) * g_ControlPoints[i - 1].x + 3 * t * t * g_ControlPoints[i].x,
					-3 * (1 - t) * (1 - t) * g_ControlPoints[i - 3].y + (9 * t * t - 12 * t + 3) * g_ControlPoints[i - 2].y + (-9 * t * t + 6 * t) * g_ControlPoints[i - 1].y + 3 * t * t * g_ControlPoints[i].y
				);		
				glBegin(GL_LINE_STRIP);
				glVertex2d(beizier.x, beizier.y);
				g_diff.normalize();
				g_diff = g_diff * 70;
				g_diff.set(beizier.x + g_diff.y, beizier.y -g_diff.x);
				glVertex2d(g_diff.x, g_diff.y);
				glEnd();
			}
		}
	}
